...
impl Benchmark {
    ...
    {
        let mut csprg = OsRng {};
        let signing_keys: Vec<SigningKey> = (0..self.num_of_validators)
            .map(|_| SigningKey::generate(&mut csprg))
            .collect();
        ...
        for (i, sk) in signing_keys.into_iter().enumerate() {
            ...
            let prover_builder = FridaProverBuilder::<BaseElement, Blake3_256<BaseElement>>::new(
                self.fri_options.clone(),
            );
        ...
        let proof_statistics = Some(defrida_proof_calculation(&proof_store));

        // generate_report(
        //     reporting_file_path,
        //     self.num_of_validators as u64,
        //     self.fri_options.clone(),
        //     self.data_sizes.clone(),
        //     proof_statistics,
        // );
