...
impl DefridaSideNetwork {
    pub fn start(
        ...
        thread::spawn(move || {
            let side = DefridaSideNetwork { rx, peers, share_store };
            side.run();
        });
    }
                ...
                match message {
                    ...
                    DefridaNetworkMessage::RequestShare(data_hash, validator_vk) => {
                        ...
                        if let Some(peer_tx) = self.peers.get(&sender_vk) {
                            let found_msg = if share.is_some() { "Found and sending" } else { "Share not found for" };
                            println!("[Side Network] {} share to {}", found_msg, short_key(&sender_vk));
                            let _ = peer_tx.send((sender_vk, DefridaNetworkMessage::ShareResponse(share)));
                        }
                    }
                    _ => {}
                }
